                                ...
                                querySnapshot.forEach(remake => {
                                    ...
                                    if(stockDate != checkDate){
                                        ...
                                        db.collection('product').where('product_id', '==', remake.data().product_id).get().then(querySnapshot => {
                                            querySnapshot.forEach(product => {
                                                total[checkDate] = product.data().product_price;
                                            });
                                        });
                                        ...
                                        db.collection('product').where('product_id', '==', remake.data().product_id).get().then(querySnapshot => {
                                            querySnapshot.forEach(product => {
                                                total[checkDate] = total[checkDate] + product.data().product_price;
                                            });
                                        });
                                    }
                                    ...
                                    db.collection('product').where('product_id', '==', remake.data().product_id).get().then(querySnapshot => {
                                        querySnapshot.forEach(product => {
                                            ...
                                            product_price.innerHTML = "小計：￥" + price.toLocaleString();
                                    ...
                                    if(cnt == check_cnt[checkDate]){
                                        db.collection('product').where('product_id', '==', remake.data().product_id).get().then(querySnapshot => {
                                            querySnapshot.forEach(product => {
                                                ...
                                                total_price.innerHTML = '合計：￥' + getBillingAmount(price, 0, 0).toLocaleString();
                                                total_box.appendChild(total_price);

                                                var total_point = document.createElement("p");
                                                total_point.innerHTML = '獲得ポイント：' + getPointAmount(price) + 'pt';
